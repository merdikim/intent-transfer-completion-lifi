...
import type { BalancePosition, PluginConfig, ResolvedIntent } from "./types.js";
...
test("planTransfer skips LI.FI route when destination balance is sufficient", async () => {
  ...
  const plan = await planTransfer(
    ...
    lifiClient,
    config,
    async (_owner, asset) => (asset.symbol === "USDC" ? 75_000_000n : 1_000_000_000_000_000n)
  );
...
test("planTransfer selects a LI.FI route when the destination chain is short", async () => {
  const resolvedIntent: ResolvedIntent = {
    ...
    amountRaw: 250_000000000000000000n
  };
  ...
  const plan = await planTransfer(
    ...
    lifiClient,
    config,
    async (_owner, asset) => (asset.symbol === "DAI" ? 0n : 1_000_000_000_000_000n)
  );

  assert.equal(plan.shortfallRaw, 250_000000000000000000n);
