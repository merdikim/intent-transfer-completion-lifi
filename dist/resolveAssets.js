...
export async function resolveRecipient(recipient, config) {
...
export async function resolveAsset(tokenSymbol, chainKey, lifiClient) {
    ...
    const lifiToken = (await lifiClient.getTokens(chainId)).find((token) => token.symbol.toUpperCase() === tokenSymbol.toUpperCase());
    if (!lifiToken) {
