...
const INTENT_PATTERN = /^(send|transfer)\s+([0-9]+(?:\.[0-9]+)?)\s*([a-zA-Z0-9]+)\s+to\s+([^\s]+)\s+on\s+([a-zA-Z0-9 ]+)$/i;
export function parseIntent(intent) {
    const normalized = intent.trim().replace(/\s+/g, " ");
    const match = normalized.match(INTENT_PATTERN);
    if (!match) {
        throw new IntentParseError("Intent must look like 'send 50usdc to merkim.eth on base' or 'transfer 250 dai to 0xabc... on optimism'.");
    }
