...
test("planTransfer throws when there are no spendable balances", async () => {
    await assert.rejects(() => planTransfer(resolvedIntent, ownerAddress, [], 0n), (error) => {
        assert.ok(error instanceof InsufficientFundsError);
        assert.equal(error.message, "No spendable balances were found across configured chains.");
        return true;
    });
});
test("planTransfer throws when only the target asset exists but is still insufficient", async () => {
    ...
    await assert.rejects(() => planTransfer(resolvedIntent, ownerAddress, balances, 3000000n), (error) => {
        assert.ok(error instanceof InsufficientFundsError);
        assert.equal(error.message, "Not enough funds available. Only USDC was found, but the available balance does not cover the requested amount.");
        return true;
    });
});
//# sourceMappingURL=routePlanner.test.js.map